...
// clk = 85.91 MHz (bus and VDP side), clk_eng = 42.95 MHz (engine).
module geo3d_pnr_top (input clk, input clk_eng, input rst_n, input sin, input ce_in,
                      output sout, output busy);
    reg [15:0] sh;
    always @(posedge clk) sh <= {sh[14:0], sin};
    wire [7:0] rdata; wire rdata_en, hit, ready, cmd_wr; wire [5:0] cmd_num; wire [7:0] cmd_data;
    geo3d_bus u (
        .clk(clk), .clk_eng(clk_eng), .reset_n(rst_n),
