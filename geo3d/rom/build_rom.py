...
def main():
    ...
    subprocess.run(["z80asm", "-o", os.path.join(OUT, "bank0.bin"),
                    "--label=" + os.path.join(OUT, "labels.txt"), "geo3d_rom.asm"], cwd=HERE, check=True)
    bank0 = open(os.path.join(OUT, "bank0.bin"), "rb").read()
    ...
    open(os.path.join(OUT, "GEO3D.ROM"), "wb").write(rom)
    ...
    print(f"GEO3D.ROM: {len(rom) // 1024} KB (ASCII16, {size} bancos de 16 KB), "
